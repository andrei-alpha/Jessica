...
class Kernel:
    ...
    def __init__(self):
        ...
        self._botPredicates = {}
        #self.setBotPredicate("master", "alpha")
        
        # set up the word substitutors (subbers):
        self._subbers = {}
       
        self.settings_costum("ai/static/aimldb/Settings.xml")
        self.subs_costum("ai/static/aimldb/Substitutions.xml")
    ...
    def settings_costum(self, filename):
        f = open(filename, 'r')
        for line in f:
            m = re.search('name="(?P<name>[^"]*)"\svalue="(?P<val>[^"]*)"',line)
            try: self.setBotPredicate(m.group('name'), m.group('val') )
            except: pass
    
    def subs_costum(self, filename):
        f = open(filename, 'r')
        for line in f:
            m = re.search('name="(?P<name>[^"]*)" value="(?P<val>[^"]*)"',line)
            try: DefaultSubs.defaultNormal[  m.group('name') ] = m.group('val')
            except: pass
    ...
    def _respond(self, input, sessionID):
        ...
        if elem is None:
            if self._verboseMode:
                err = "Huh?" #"WARNING: No match found for input: %s\n" % input.encode(self._textEncoding)
                sys.stderr.write(err)
        else:
...
if __name__ == "__main__":
    ...
    _testTag(k, 'unicode support', u"wtf", [u"Hey, you speak Chinese! wtf"])
